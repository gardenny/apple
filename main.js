...
(() => {
  ...
  function scrollLoop() {
    ...
    if (delayedScrollY > prevScrollHeight + sceneInfo[currentScene].scrollHeight) {
      ...
      // 브라우저 바운스 효과로 인해 currentScene이 음수가 나오는 것을 방지 (모바일)
  ...
  window.addEventListener('load', () => {
    ...
    // temporary
    // 스크롤값에 따라 모든 레이아웃을 잡아주고 있기 때문에
    // 페이지 중간에서 새로 고침 시 레이아웃이 제대로 잡히지 않는 이슈 발생,
    // 새로 고침 시 자동으로 10px 정도 스크롤 되게 하여 레이아웃이 정확하게 잡히도록 세팅
